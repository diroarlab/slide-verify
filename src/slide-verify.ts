...
function drawPieceInsideShadow(
  ctx: { drawImage: (arg0: HTMLCanvasElement, arg1: number, arg2: number) => void },
  x: any,
  y: any
) {
  // 第一步生成一个 piece 图形模板
...
function drawBlock(img: HTMLImageElement, ctx: CanvasRenderingContext2D, x: number, y: number) {
...
export default class SlideVerify {
  ...
  bindEvents() {
    ...
    const handleDragEnd = (e: MouseEvent | TouchEvent) => {
      ...
      if (spliced) {
        ...
        (<HTMLElement>(
          this.sliderIcon.childNodes[0]
        )).innerHTML = `<i class="fas fa-times" aria-hidden="true"></i>`;
        addClass(this.sliderContainer, styles.sliderContainer_fail);
  ...
  reset() {
    ...
    (<HTMLElement>(
      this.sliderIcon.childNodes[0]
    )).innerHTML = `<i class="fas fa-bars fa-rotate-90" aria-hidden="true"></i>`;
